...
export default {
    ...
    flatBookmarks: (state: BookmarksState): BookmarkNode[] => flattenBookmarkLinks(
        (state.bookmarks ?? []).flatMap((folder) => (folder.children ?? []) as BookmarkNode[]),
    ),
    ...
    bookmarksCount: (state: BookmarksState): number => (state.bookmarks ?? [])
        .reduce(
            (total, folder) => total + flattenBookmarkLinks(folder.children ?? []).length,
            0,
        ),
};
